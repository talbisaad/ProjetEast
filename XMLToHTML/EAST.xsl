...
<!-- Auteur : Didier Courtaud ( CEA & Université d'Evry )  sur une idée originale de Damien Guillaume ( Observatoire de Paris ) -->
...
<xsl:template match="PAGE_TITRE" >
		...
		<tr><td><h5 class="pagetitre"> <xsl:apply-templates select="AUTEUR"/></h5></td></tr>
		</tbody>
		<tfoot>
...
<xsl:template match="PAGE_CONCLUSION" >
		...
		<tbody>
			...
			<xsl:for-each select="PARAGRAPHE">
				...
				<tr><td><h1><xsl:value-of select="."/></h1></td></tr>
				
			</xsl:for-each>
			
			<xsl:for-each select="EMAIL">
				
				<tr><td><h2><xsl:value-of select="."/></h2></td></tr>
...
<xsl:template match="EAST">
	...
	<head>
		...
		<!-- <link rel="stylesheet" type="text/css" href="config_EAST/EAST_theme.css"><xsl:text> </xsl:text></link> -->
		<link rel="timesheet" type="application/smil+xml" href="config_EAST/EAST.smil"><xsl:text> </xsl:text></link>
		<link rel="stylesheet" href="config_EAST/Highlight/styles/default.css"></link>

		
		<script type="text/javascript" src="config_EAST/EAST.js"><xsl:text> </xsl:text></script>
		<script type="text/javascript" src="config_EAST/BrowserDetect.js"><xsl:text> </xsl:text></script>
	...
	<xsl:if test="PAGE_CONCLUSION"> <xsl:apply-templates select="PAGE_CONCLUSION"/> </xsl:if>
...
.conclusion { page-break-after: always; background: #F5F5FF; margin: 0.5em; padding: 0.5em; <xsl:if test="$impression='non'">border: gray outset</xsl:if> }
...
.elsommaire { font-size: 110%; color: <xsl:value-of select="$colorpage"/> ; text-decoration: none; font-weight: bold }T0530
...
<xsl:if test="/EAST/PREFERENCES/PAGE/@linkcolor!=''">.linkitem { color: <xsl:value-of select="/EAST/PREFERENCES/PAGE/@linkcolor"/> }</xsl:if>
...
#navigation_par #session_rec {

	display:none

}
#navigation_par #session_export {

	display:none

}
#navigation_par #session_import {
...
div#erreur {
	...
	<div class="slide" id="slide{$numero}">
		...
		<xsl:variable name="secbackcolor">
		
			<xsl:choose>
				
				<xsl:when test="normalize-space(@backcolor)">
					
					<xsl:value-of select="normalize-space(@backcolor)"/>
		...
		<xsl:variable name="sec_id"><xsl:value-of select="concat('slide',$numero)"/></xsl:variable>
		
		<script type="text/javascript">
				
				
			var <xsl:value-of select="$sec_id"/> = document.getElementById("<xsl:value-of select="$sec_id"/>");
					
			
					
			<xsl:if test="$secbackcolor!=''">
						
				<xsl:value-of select="$sec_id"/>.style.backgroundColor="<xsl:value-of select="$secbackcolor"/>";
				
			</xsl:if> 
					
			<xsl:if test="$secimage!=''">
				
				<xsl:value-of select="$sec_id"/>.style.backgroundImage="url('<xsl:value-of select="$secimage"/>')";
				<xsl:value-of select="$sec_id"/>.style.backgroundSize="<xsl:value-of select="'cover'"/>";
    ...
    <xsl:template match="CODE_HIGH">
	
	<pre><code><xsl:value-of select="." disable-output-escaping="no"/></code></pre>
	...
	<xsl:variable name="id_span"> <xsl:value-of select="concat('se',$npart,$nsect,$nlist,$nitem)"/> </xsl:variable>
	
	<xsl:variable name="emphcolor">
		
		<xsl:choose>
			
			<xsl:when test="normalize-space(@textcolor)">
				
				<xsl:value-of select="normalize-space(@textcolor)"/>
	...
	<span id="{$id_span}"><xsl:apply-templates/></span>
	
	<script type="text/javascript">
				
				
		var <xsl:value-of select="$id_span"/> = document.getElementById("<xsl:value-of select="$id_span"/>");
				
		<xsl:value-of select="$id_span"/>.style.fontStyle = "italic"; 
				
		<xsl:if test="$emphfont!=''">
					
			<xsl:value-of select="$id_span"/>.style.fontFamily="<xsl:value-of select="$emphfont"/>";
			
		</xsl:if> 
				
		<xsl:if test="$emphcolor!=''">
			
			<xsl:value-of select="$id_span"/>.style.color="<xsl:value-of select="$emphcolor"/>";
	...
	<xsl:if test="count(child::text()) != 0">
			...
			<a href="{@href}" class="linkitem"><xsl:apply-templates/></a>
	</xsl:if>
